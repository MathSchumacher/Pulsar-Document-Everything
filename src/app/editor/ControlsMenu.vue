...
// PrimeVue passthrough simplified
const pt = {
...
// Array to dynamically generate color pickers
type ColorName = keyof IDocumentationColorPalette;
const colors: ColorName[] = [
  'background', 'primary', 'secondary', 'text', 'divider',
  'codeBlockText', 'codeBlockLiteral', 'codeBlockKeyword',
  'codeBlockSection', 'codeBlockString', 'codeBlockVariable'
];
...
async function handleSave() {
  if (!editor.value.controlsMenu.isSaved) {
    ...
    const result = await Documentation.edit(docId, JSON.parse(JSON.stringify(editor.value.doc)));
    if (result === Status.OK) editor.value.controlsMenu.isSaved = true;
...
watch(() => editor.value.doc, async (_, oldDocData) => {
  ...
  editor.value.controlsMenu.isSaved = lodash.isEqual(oldDocData, docInfos);
}, { deep: true });
...
onBeforeMount(async () => {
  ...
  window.addEventListener('resize', () => {
    if (window.innerWidth >= 1180) isOpen.value = true;
  });
...
<template>
  <div class="max-2xl:w-full">
    <!--Mobile navbar-->
    <div :class="`${isOpen? 'opacity-0 -mt-16' : ''} 2xl:hidden flex items-center justify-between px-5 w-full h-[60px] bg-secondary duration-300`">
    ...
    <!--Menu-->
    <ScrollPanel 
      :class="`${isOpen? 'max-2xl:fixed' : 'opacity-0 pointer-events-none'} relative max-2xl:fixed max-2xl:left-0 max-2xl:top-0 min-w-[330px] max-w-[330px] max-md:max-w-[330px] max-md:min-w-[0px] bg-secondary 2xl:bg-secondary/70 h-screen z-[201]`"
      :pt="pt"
    >
      <form @submit.prevent="handleSave" class="p-8">
        <!--Back button and mobile close-->
            ...
            <div class="w-full flex flex-col flex-wrap gap-6 mt-2.5">
              <div v-for="color of colors" :key="color" class="w-full flex flex-col">
                <div class="w-full flex items-center justify-between gap-2">
                  ...
                  <HexColorPicker :model-value="editor.doc.colors[color]" @update:model-value="(val: string) => onColorChange(color, val)" />
                </div>
              </div>
            </div>
          </div>
    ...
    <!--Backdrop mobile-->
    <div @click="isOpen = false" :class="`2xl:hidden ${!isOpen && 'opacity-0 pointer-events-none'} fixed left-0 top-0 w-screen h-screen bg-[#00000060] z-[200]`"></div>
  </div>
</template>
